<script>
    import { onMount } from 'svelte';
    import { page } from '$app/stores';
    import projects from '$lib/data/projects.js';
    import BackButton from '$lib/components/BackButton.svelte';
    import Tag from '$lib/components/Tag.svelte';

    export let data;
    
    $: projectId = data.projectId;
    $: project = projects.find((p) => p.id === projectId);

    onMount(() => {
        window.scrollTo(0, 0);
    });

    function handleImageError(e) {
        e.target.src = 'https://via.placeholder.com/1200x600?text=Projet';
    }

    function openScreenshot(screenshot) {
        window.open(screenshot, '_blank');
    }
</script>

<div class="min-h-screen bg-gray-900 text-white">
    {#if !project}
        <BackButton to="/#projects" text="Retour à l'accueil" />
        <div class="max-w-4xl mx-auto mt-12 text-center pt-20">
            <h1 class="text-3xl font-bold mb-4">Projet non trouvé</h1>
            <p class="text-gray-400">
                Le projet que vous recherchez n'existe pas ou a été supprimé.
            </p>
        </div>
    {:else}
        <!-- Hero Section -->
        <div class="relative h-80 md:h-96">
            <div class="absolute inset-0">
                <img
                    src={project.image || 'https://via.placeholder.com/1200x600?text=Projet'}
                    alt={project.title}
                    class="w-full h-full object-cover"
                    on:error={handleImageError}
                />
                <div class="absolute inset-0 bg-black opacity-60"></div>
            </div>
            <div class="absolute inset-0 flex items-center justify-center">
                <div class="text-center px-4">
                    <h1 class="text-4xl md:text-5xl font-bold mb-4">
                        {project.title}
                    </h1>
                    <!-- Compétences -->
                    <div class="flex flex-wrap justify-center gap-2 mb-4">
                        {#if project.skillsInfo}
                            {#each project.skillsInfo as skill, index}
                                <Tag {skill} linkable={true} />
                            {/each}
                        {/if}
                    </div>
                    {#if project.link}
                        <a
                            href={project.link}
                            target="_blank"
                            rel="noopener noreferrer"
                            class="inline-flex items-center px-4 py-2 bg-blue-700 hover:bg-blue-600 text-white rounded-md transition-colors"
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                class="h-5 w-5 mr-2"
                                fill="currentColor"
                                viewBox="0 0 24 24"
                            >
                                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                            </svg>
                            Voir le code source
                        </a>
                    {/if}
                </div>
            </div>
        </div>

        <!-- Content -->
        <div class="max-w-4xl mx-auto py-12 px-4">
            <BackButton to="/#projects" text="Retour à l'accueil" />

            <div class="mt-8 bg-gray-800 rounded-lg p-6 shadow-lg">
                <h2 class="text-2xl font-bold mb-4">Description du projet</h2>
                <p class="text-gray-300 mb-6">{project.description}</p>

                <!-- Functionalities -->
                {#if project.features && project.features.length > 0}
                    <div class="mt-8">
                        <h3 class="text-xl font-bold mb-4">Fonctionnalités principales</h3>
                        <ul class="list-disc list-inside text-gray-300 space-y-2">
                            {#each project.features as feature}
                                <li>{feature}</li>
                            {/each}
                        </ul>
                    </div>
                {/if}

                <!-- Challenges -->
                {#if project.challenges}
                    <div class="mt-8">
                        <h3 class="text-xl font-bold mb-4">Défis techniques</h3>
                        <p class="text-gray-300">{project.challenges}</p>
                    </div>
                {/if}

                <!-- Screenshots -->
                {#if project.screenshots && project.screenshots.length > 0}
                    <div class="mt-8">
                        <h3 class="text-xl font-bold mb-4">Captures d'écran</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {#each project.screenshots as screenshot, index}
                                <div class="overflow-hidden rounded-lg shadow-lg">
                                    <img
                                        src={screenshot}
                                        alt="{project.title} - Capture d'écran {index + 1}"
                                        class="w-full h-auto object-cover transition-transform duration-300 hover:scale-105 cursor-pointer"
                                        on:click={() => openScreenshot(screenshot)}
                                        on:keydown={(e) => e.key === 'Enter' && openScreenshot(screenshot)}
                                        role="button"
                                        tabindex="0"
                                    />
                                </div>
                            {/each}
                        </div>
                    </div>
                {/if}
            </div>
        </div>
    {/if}
</div>
